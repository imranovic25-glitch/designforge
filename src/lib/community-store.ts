...
function normalizeCommunityScreenshotUrl(rawUrl: string): string {
  ...
  const bucketPrefix = "community-screenshots/";
  const normalizedPath = trimmed
    .replace(/^\/+/, "")
    .replace(/^storage\/v1\/object\/public\/community-screenshots\//, "")
    .replace(new RegExp(`^${bucketPrefix}`), "");

  return supabase.storage.from("community-screenshots").getPublicUrl(normalizedPath).data.publicUrl;
}
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed
        ...
        .map((url) => normalizeCommunityScreenshotUrl(url as string));
    }
  } catch {
    // not JSON — treat as single URL
  }
  return raw.trim() ? [normalizeCommunityScreenshotUrl(raw)] : [];
}
...
export async function getTotalMemberCount(): Promise<number> {
  const { data, error } = await supabase.rpc("get_community_member_count");
