...
CREATE TABLE IF NOT EXISTS community_repo_events (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE (user_id, reason, submission_id, feedback_id)
);
...
BEGIN
  ...
  SELECT user_id, status, slots_filled, tester_slots
    INTO v_owner, v_status, v_slots_filled, v_tester_slots
  ...
  IF v_status <> 'active' OR v_slots_filled >= v_tester_slots THEN
    RAISE EXCEPTION 'slots_full_or_closed';
  END IF;
  ...
  RETURNING *, (xmax = 0) INTO v_feedback, v_inserted;

  -- Only the FIRST feedback should fill a slot + award repos
  IF v_inserted THEN
    PERFORM fill_tester_slot(p_submission_id);
