...
async function renderRoute(browser, route) {
  ...
  try {
    ...
    /* Clean up duplicate meta/link tags: react-helmet-async adds tags with
       data-rh="true" but doesn't remove the originals from static index.html.
       We remove non-helmet duplicates so only the page-specific tags remain. */
    await page.evaluate(() => {
      const rhTags = document.querySelectorAll("[data-rh]");
      rhTags.forEach((rh) => {
        const tag = rh.tagName.toLowerCase();
        if (tag === "title") {
          document
            .querySelectorAll("title:not([data-rh])")
            .forEach((t) => t.remove());
        }
        if (tag === "meta") {
          const name = rh.getAttribute("name");
          const prop = rh.getAttribute("property");
          if (name)
            document
          ...
          if (prop)
            document
              .querySelectorAll(`meta[property="${prop}"]:not([data-rh])`)
              .forEach((m) => m.remove());
        }
        if (tag === "link" && rh.getAttribute("rel") === "canonical") {
          document
