...
CREATE TABLE IF NOT EXISTS public.chat_threads (
  ...
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- Ensure one thread per pair (ordered so p1 < p2)
...
-- Enable Realtime for chat_messages (so subscriptions work)
ALTER PUBLICATION supabase_realtime ADD TABLE public.chat_messages;

-- ═══════════════════════════════════════════
-- 4. Get-or-Create Thread RPC
--    Always stores the smaller UUID as participant_1
--    Returns the thread with other_user enrichment
-- ═══════════════════════════════════════════
...
BEGIN
  ...
  -- Get other user's display info from their latest submission or feedback
  SELECT COALESCE(
...
-- ═══════════════════════════════════════════
-- 5. Auto-remove posts after 3 reports
--    Updates the report_submission function
--    to check count and auto-close/remove
-- ═══════════════════════════════════════════
...
BEGIN
  ...
  -- Count total unique reports for this submission
  SELECT COUNT(*) INTO v_count
  FROM community_reports
  WHERE submission_id = p_submission_id;

  -- Auto-remove: if 3 or more unique users reported, delete the submission
  IF v_count >= 3 THEN
