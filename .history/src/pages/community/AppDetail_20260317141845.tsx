...
import { ArrowLeft, ArrowUp, ExternalLink, Clock, MessageSquare, User, Users, BarChart2, Share2, Flag, Star, MessageCircle, X, ChevronLeft, ChevronRight, LinkIcon, Send } from "lucide-react";
...
export function AppDetail() {
  ...
  return (
    <>
      <SEOHead
        title={`${app.title} — Community`}
        description={`Community comments on ${app.title}. ${app.description.slice(0, 120)}`}
      ...
      <div className="min-h-screen pt-28 pb-20">
        <div className="container mx-auto px-4 lg:px-6 max-w-[1200px]">
          ...
          >
                ...
                <div className="mb-6">
                  <div className={`grid gap-3 ${screenshots.length === 1 ? "grid-cols-1" : screenshots.length === 2 ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-3"}`}>
                    {screenshots.map((url, i) => (
                      <button
                        key={i}
                        onClick={() => { setLightboxIdx(i); setLightboxImg(url); }}
                        className="rounded-2xl overflow-hidden border border-white/[0.06] hover:border-white/[0.15] transition-all cursor-pointer"
                      >
                        <img
                          src={url}
            ...
            {/* Actions — Reddit order: Comment, Share, Try App */}
            <div className="flex items-center gap-3 flex-wrap mb-12">
              {/* Comment / Give Feedback — primary action */}
              {user && !isOwner && !feedbackLocked && (
                <button
                  onClick={() => setShowFeedbackForm(!showFeedbackForm)}
                  className="inline-flex items-center gap-2 bg-white/[0.06] hover:bg-white/[0.1] text-white/80 px-5 py-3 rounded-xl text-sm font-medium border border-white/[0.08] transition-all"
                >
                  <MessageSquare size={15} />
                  {alreadyGaveFeedback ? "Edit Feedback" : "Comment / Feedback"}
                </button>
              )}
      ...
      <AnimatePresence>
        {lightboxImg && (() => {
          ...
          return (
            ...
            >
              <button
                className="absolute top-4 right-4 p-2 rounded-full bg-white/10 text-white/70 hover:bg-white/20 hover:text-white transition-all z-50"
                onClick={() => setLightboxImg(null)}
              >
                <X size={22} />
              </button>
