...
import { ArrowLeft, ArrowUp, ExternalLink, Clock, MessageSquare, User, Users, BarChart2, Share2 } from "lucide-react";
...
import { CATEGORY_LABELS } from "@/src/lib/community-types";
...
export function AppDetail() {
  ...
  const handleFeedbackSubmit = async (data: { rating: number; feedback_text: string; areas: string[]; device_info: string }) => {
    ...
    if (result) {
      ...
      setFeedbackError(
        "Feedback could not be submitted. If you recently changed the repo system, run supabase/community_repos.sql in Supabase SQL Editor, then refresh."
      );
    }
  };
