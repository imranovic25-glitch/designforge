...
import { ArrowLeft, MessageSquare, ArrowUp, Trash2, ToggleLeft, ToggleRight, Pencil, Pin, Zap, Star } from "lucide-react";
...
import { getUserSubmissions, updateSubmissionStatus, deleteSubmission, updateSubmission, pinSubmission, boostSubmission, requestPriorityReview } from "@/src/lib/community-store";
...
export function MyApps() {
  ...
  const [featureMsg, setFeatureMsg] = useState<{ id: string; msg: string; type: "success" | "error" } | null>(null);

  const handlePin = async (id: string) => {
    const result = await pinSubmission(id);
    if (result.success) {
      setFeatureMsg({ id, msg: "Pinned for 24 hours!", type: "success" });
      setApps((prev) => prev.map((a) => a.id === id ? { ...a, is_pinned: true } : a));
    } else {
      setFeatureMsg({ id, msg: result.error === "insufficient_repos" ? "Not enough repos (need 5)" : (result.error ?? "Failed"), type: "error" });
    }
    setTimeout(() => setFeatureMsg(null), 3000);
  };

  const handleBoost = async (id: string) => {
    const result = await boostSubmission(id);
    if (result.success) {
      setFeatureMsg({ id, msg: "Boosted for 48 hours!", type: "success" });
      setApps((prev) => prev.map((a) => a.id === id ? { ...a, is_boosted: true } : a));
    } else {
      setFeatureMsg({ id, msg: result.error === "insufficient_repos" ? "Not enough repos (need 25)" : (result.error ?? "Failed"), type: "error" });
    }
    setTimeout(() => setFeatureMsg(null), 3000);
  };

  const handlePriority = async (id: string) => {
    const result = await requestPriorityReview(id);
    if (result.success) {
      setFeatureMsg({ id, msg: "Priority review requested!", type: "success" });
      setApps((prev) => prev.map((a) => a.id === id ? { ...a, priority_review: true } : a));
    } else {
      setFeatureMsg({ id, msg: result.error === "insufficient_repos" ? "Not enough repos (need 10)" : result.error === "already_requested" ? "Already requested" : (result.error ?? "Failed"), type: "error" });
    }
    setTimeout(() => setFeatureMsg(null), 3000);
  };
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 flex-wrap">
                        ...
                        {(app as any).is_pinned && (
                          <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 flex items-center gap-0.5"><Pin size={8} /> Pinned</span>
                        )}
                        {(app as any).is_boosted && (
                          <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-pink-500/10 text-pink-400 flex items-center gap-0.5"><Zap size={8} /> Boosted</span>
                        )}
                        {(app as any).priority_review && (
                          <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 flex items-center gap-0.5"><Star size={8} /> Priority</span>
                        )}
                      </div>
                        ...
                        <div className="flex items-center gap-1.5 mt-2">
                          {!(app as any).is_pinned && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-md border border-amber-500/20 text-amber-400/70 hover:bg-amber-500/10 hover:text-amber-400 transition-all"
                            >
                              <Pin size={10} /> Pin 24h <span className="text-white/25">· 5</span>
                            </button>
                          )}
                          {!(app as any).priority_review && (
                            <button
                              onClick={() => handlePriority(app.id)}
                              className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-md border border-blue-500/20 text-blue-400/70 hover:bg-blue-500/10 hover:text-blue-400 transition-all"
                            >
                              <Star size={10} /> Priority <span className="text-white/25">· 10</span>
                            </button>
                          )}
                          {!(app as any).is_boosted && (
                            <button
                              onClick={() => handleBoost(app.id)}
                              className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-md border border-pink-500/20 text-pink-400/70 hover:bg-pink-500/10 hover:text-pink-400 transition-all"
                            >
                              <Zap size={10} /> Boost <span className="text-white/25">· 25</span>
                            </button>
                          )}
                        </div>
                      )}
                      ...
                      {featureMsg?.id === app.id && (
                        <p className={`text-[10px] mt-1 ${featureMsg.type === "success" ? "text-emerald-400" : "text-red-400"}`}>
                          {featureMsg.msg}
                        </p>
                      )}
                    </div>
