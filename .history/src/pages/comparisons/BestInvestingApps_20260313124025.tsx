            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
              <div className="flex items-start gap-6">
                <div className="relative shrink-0">
                  <div className={`absolute -top-3 -left-3 z-10 flex items-center justify-center rounded-full text-xs font-bold text-white shadow-lg ${index === 0 ? 'w-9 h-9 bg-gradient-to-br from-yellow-400 to-amber-600 ring-2 ring-yellow-400/40' : 'w-8 h-8 bg-indigo-500'}`}>{index + 1}</div>
                  <div className={`rounded-2xl ${index === 0 ? 'ring-2 ring-indigo-400/30 shadow-[0_0_30px_rgba(99,102,241,0.15)]' : ''}`}>
                    <BrandLogo src={platform.logo} name={platform.name} className="w-20 h-20 rounded-2xl bg-white/95 p-2 object-contain" />
                  </div>
                </div>
                <div>
