...
function GenericGuideVis({ color, label, lines }: { color: string; label: string; lines: number[] }) {
  return (
    <div className="w-full h-full relative overflow-hidden" style={{ background: `linear-gradient(160deg,${color}12,#0f0f0f 50%,${color}0a)` }}>
      <div className="absolute top-0 right-0 w-32 h-32 pointer-events-none" style={{ background: `radial-gradient(ellipse,${color}40 0%,transparent 70%)`, filter: "blur(24px)" }} />
      <div className="absolute bottom-0 left-0 w-24 h-24 pointer-events-none" style={{ background: `radial-gradient(ellipse,${color}20 0%,transparent 70%)`, filter: "blur(20px)" }} />
      <div className="absolute inset-0 flex flex-col items-center justify-center px-6 gap-2">
        <motion.div className="flex items-center gap-1.5 mb-1"
          ...
          <BookOpen className="h-3.5 w-3.5" style={{ color: `${color}bb` }} />
          <span className="text-[8px] font-bold tracking-wider uppercase" style={{ color: `${color}99` }}>{label}</span>
        </motion.div>
        {lines.map((w, i) => (
          <motion.div key={i} className="h-[3px] rounded-full self-start" style={{ width: `${w}%`, background: i === 0 ? `${color}80` : `${color}25` }}
...
function BgRemoveGuideVis() {
  return (
    <div className="w-full h-full relative overflow-hidden" style={{ background: "linear-gradient(160deg,#150610,#0f0f0f 50%,#12050c)" }}>
      <div className="absolute bottom-0 left-0 w-40 h-40 pointer-events-none" style={{ background: "radial-gradient(ellipse,rgba(244,63,94,0.25) 0%,transparent 70%)", filter: "blur(24px)" }} />
...
function CompoundGuideVis() {
  return (
    <div className="w-full h-full relative overflow-hidden" style={{ background: "linear-gradient(160deg,#06150e,#0f0f0f 50%,#061209)" }}>
      <div className="absolute bottom-0 right-0 w-40 h-40 pointer-events-none" style={{ background: "radial-gradient(ellipse,rgba(16,185,129,0.3) 0%,transparent 70%)", filter: "blur(24px)" }} />
...
function CreditCardGuideVis() {
  return (
    <div className="w-full h-full relative overflow-hidden" style={{ background: "linear-gradient(160deg,#150e06,#0f0f0f 50%,#140c07)" }}>
      <div className="absolute top-0 right-0 w-32 h-32 pointer-events-none" style={{ background: "radial-gradient(ellipse,rgba(245,158,11,0.3) 0%,transparent 70%)", filter: "blur(24px)" }} />
  ...
  return (
    <div className="min-h-screen bg-black pt-32 pb-24 selection:bg-sky-300/30 selection:text-white">
      <SEOHead
        ...
        faqItems={[
          { question: "What topics do DesignForge360 guides cover?", answer: "Guides cover PDF tools (compression, merging, conversion), image processing (background removal, resizing), finance (compound interest, loan EMI, mortgage, credit cards), and developer tools (JSON formatting, Markdown, password security, SEO analysis)." },
          { question: "Are the guides free to read?", answer: "Yes. All guides on DesignForge360 are free to read with no subscription, sign-up, or paywall." },
          { question: "How long are the guides?", answer: "Most guides are 800–2,000 words with step-by-step instructions, screenshots, and FAQs. They're designed to be actionable — you can follow along while using the related tool." },
        ]}
      />
