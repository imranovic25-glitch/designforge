  ...
  return (
    <>
      <SEOHead
        ...
        description={`${displayName}'s DesignForge360 profile. Community Repo Points: ${repoBalance}. ${reviews.length} tool review${reviews.length !== 1 ? "s" : ""} submitted. Joined ${memberSince ?? "recently"}.`}
        canonical="/profile"
        robots="noindex, nofollow"
        schema="ProfilePage"
        profileName={`${displayName} on DesignForge360`}
        profileDescription={`${displayName} is a member of the App Testers Community on DesignForge360 with ${repoBalance} Repo Points.`}
      />
      ...
      <div className="min-h-screen pt-32 pb-24">
        <div className="max-w-3xl mx-auto px-6">
          ...
          >
            <div className="bg-emerald-500/10 px-6 py-3 border-b border-white/[0.06]">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              { label: "Tools Reviewed", value: reviews.length.toString(), icon: <Star size={16} className="text-amber-400" /> },
              { label: "Avg Rating", value: avgRating > 0 ? `${avgRating} / 5` : "—", icon: <Award size={16} className="text-blue-400" /> },
              { label: "Community Repos", value: repoBalance.toString(), icon: <Coins size={16} className="text-emerald-400" /> },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
