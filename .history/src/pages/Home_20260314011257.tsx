...
/** Editorial visual — AI writing interface mockup */
function AiWritingViz() {
  const tokens = ["Craft", " compelling", " copy", " that", " converts"];
  return (
    <div className="w-full h-full relative overflow-hidden"
      style={{ background: "linear-gradient(160deg,#07060e,#0a0a0a 60%,#060710)" }}>
      {/* Ambient violet glow */}
      <div className="absolute top-0 right-0 w-40 h-40 pointer-events-none"
        style={{ background: "radial-gradient(ellipse,rgba(139,92,246,0.22) 0%,transparent 70%)", filter: "blur(24px)" }} />
      {/* Prompt badge */}
      <div className="absolute top-4 left-4 flex items-center gap-1.5">
        <div className="w-4 h-4 rounded-full flex items-center justify-center"
          style={{ background: "linear-gradient(135deg,#7c3aed,#4f46e5)" }}>
          <svg width="8" height="8" viewBox="0 0 8 8" fill="white">
            <path d="M4 1 L5.5 3.5 L4 6 L2.5 3.5 Z" />
          </svg>
        </div>
        <span className="text-[9px] font-semibold tracking-widest uppercase text-violet-300/70">AI Write</span>
      </div>
      {/* Prompt input */}
      <div className="absolute top-10 left-4 right-4 rounded-lg border border-violet-500/20 bg-white/[0.04] px-3 py-2">
        <p className="text-[10px] text-white/35 font-light leading-relaxed">Write a blog intro about productivity…</p>
      </div>
      {/* Output text lines */}
      <div className="absolute top-[72px] left-4 right-4 flex flex-col gap-[5px]">
        {tokens.map((tok, i) => (
          <motion.span key={i}
            className="text-[10px] font-medium"
            style={{ color: i === 0 ? "rgba(167,139,250,0.9)" : "rgba(255,255,255,0.55)" }}
            initial={{ opacity: 0, x: -6 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.4, delay: 0.3 + i * 0.12, ease: "easeOut" }}>
            {tok}
            {i === tokens.length - 1 && (
              <motion.span className="inline-block w-[1.5px] h-[10px] bg-violet-400 ml-0.5 align-middle rounded-full"
      ...
      <div className="absolute bottom-3 left-4 right-4 flex items-center gap-3">
        {[["Words", "247"], ["Score", "94%"], ["Tone", "Pro"]].map(([label, val]) => (
          <div key={label} className="flex items-center gap-1">
            <span className="text-[8px] text-white/25">{label}</span>
            <span className="text-[9px] font-semibold text-violet-300/80">{val}</span>
          </div>
        ))}
        <motion.div className="ml-auto h-1 flex-1 rounded-full bg-white/5 overflow-hidden">
          <motion.div className="h-full rounded-full bg-violet-500/60"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black selection:bg-white/30 selection:text-white">
      ...
      <section className="relative overflow-hidden pt-40 pb-32 lg:pt-52 lg:pb-44">
        ...
        <div className="absolute inset-0 opacity-[0.015] -z-[7] pointer-events-none" style={{ backgroundImage: "radial-gradient(circle, rgba(255,255,255,0.4) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
