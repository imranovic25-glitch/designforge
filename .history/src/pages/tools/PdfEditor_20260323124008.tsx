...
const ERASER_SIZES = [{ label: "S", size: 10 }, { label: "M", size: 24 }, { label: "L", size: 48 }];
...
async function extractTextBlocks(bytes: Uint8Array, pageIndex: number, rotation: number): Promise<PdfTextBlock[]> {
  const pdf = await pdfjsLib.getDocument({ data: bytes.slice() }).promise;
  ...
  for (const item of content.items as any[]) {
    ...
    // fontSize from the text matrix
    const fontSize = Math.abs(tx[3]) || Math.sqrt(tx[0] * tx[0] + tx[1] * tx[1]) || 12;
    // Transform through viewport matrix to get CSS pixel coords
    const [va, vb, vc, vd, ve, vf] = vp.transform;
    const sx = va * tx[4] + vc * tx[5] + ve;
    const sy = vb * tx[4] + vd * tx[5] + vf;
    const w = item.width ?? fontSize * item.str.length * 0.6;
    const h = item.height ?? fontSize * 1.2;
    // Extract color from the PDF text item if available
    let color = "#000000";
    if (item.color) {
      const [cr, cg, cb] = item.color;
      color = `#${Math.round(cr * 255).toString(16).padStart(2, "0")}${Math.round(cg * 255).toString(16).padStart(2, "0")}${Math.round(cb * 255).toString(16).padStart(2, "0")}`;
    }
    // sy is the baseline bottom; text renders downward from top, so top = sy - h
    blocks.push({ str: item.str, x: sx, y: sy - h, w, h, fontSize, color });
  }
  return blocks;
}
...
export function PdfEditor() {
  ...
  const [pdfTextBlocks, setPdfTextBlocks] = useState<PdfTextBlock[]>([]);
  ...
  /* ═══════════════════════════════════════════════════════════════════
     TEXT EXTRACTION (for Edit Text mode)
     ═══════════════════════════════════════════════════════════════════ */

  useEffect(() => {
    let dead = false;
    if (tool !== "editText" || !srcBytes || !pg) { setPdfTextBlocks([]); return; }
    (async () => {
      try {
        const blocks = await extractTextBlocks(srcBytes, pg.originalIndex, pg.rotation);
        if (!dead) setPdfTextBlocks(blocks);
      } catch { if (!dead) setPdfTextBlocks([]); }
    })();
    return () => { dead = true; };
  }, [tool, srcBytes, pg?.id, pg?.originalIndex, pg?.rotation]);

  /* ─── Focus textarea when entering edit mode ─── */
  useEffect(() => {
    if (editId) {
      const ann = anns.find((a) => a.id === editId);
      if (ann?.kind === "text") setEditText(ann.text);
  ...
  const addText = useCallback((x: number, y: number) => {
    ...
    setAnns((p) => [...p, a]); setSelId(a.id); setEditId(a.id); clearDl();
  }, [pg, clearDl, pushHistory]);

  const editPdfText = useCallback((block: PdfTextBlock) => {
    if (!pg) return;
    pushHistory();
    // Create a precise whiteout that covers the original text
    const pad = 2;
    const wo: WhiteoutAnnotation = {
      kind: "whiteout", id: crypto.randomUUID(), pageId: pg.id,
      x: block.x - pad, y: block.y - pad,
      w: block.w + pad * 2, h: block.h + pad * 2,
    };
    // Create text annotation with exact PDF text properties
    const ta: TextAnnotation = {
      kind: "text", id: crypto.randomUUID(), pageId: pg.id,
      x: block.x, y: block.y, text: block.str,
      fontSize: Math.round(block.fontSize), fontFamily: "Helvetica",
      color: block.color || "#000000", bold: false, italic: false,
      underline: false, strikethrough: false, align: "left",
    };
    setAnns((a) => [...a, wo, ta]);
    setSelId(ta.id);
    setEditId(ta.id);
    // Hide the blue highlights after clicking (switch to select to edit the annotation)
    setPdfTextBlocks((prev) => prev.filter((b) => b !== block));
    clearDl();
  }, [pg, clearDl, pushHistory]);
  ...
  const renderAnn = (ann: Annotation) => {
      ...
      return (
        <div key={ann.id}
          style={{
            ...
            outline: isSel ? "2px solid #4285F4" : "none", outlineOffset: 1,
    ...
    if (ann.kind === "text") {
      ...
      // Don't show placeholder for text that hasn't been typed yet and isn't being edited
      const showPlaceholder = !ann.text && isEd;
      const hasContent = !!ann.text;
      return (
        <div key={ann.id}
          style={{
            ...base, ...textStyles,
            minWidth: isEd ? Math.max(80, 30) * zoom : undefined,
            whiteSpace: "pre-wrap", wordBreak: "break-word",
            padding: 0,
            outline: isEd ? "2px solid #4285F4" : isSel ? "1px dashed #4285F4" : "none",
            outlineOffset: isEd ? 2 : 1,
            background: isEd ? "rgba(255,255,255,0.97)" : "transparent",
            ...
            // Hide empty text annotations that aren't being edited (no placeholder clutter)
            display: (!hasContent && !isEd) ? "none" : undefined,
          ...
          {isEd ? (
            <textarea
              ...
              onChange={(e) => {
                ...
                updateAnn(ann.id, { text: e.target.value });
              }}
              onBlur={() => {
                const finalText = editText.trim();
                updateAnn(ann.id, { text: finalText });
                pushHistory();
                setEditId(null);
                // Remove empty text annotations on blur (cleanup)
                if (!finalText) {
              ...
              className="block w-full resize-none border-none bg-transparent outline-none"
              style={{
                ...textStyles,
                minHeight: ann.fontSize * zoom * 1.3,
                minWidth: 100 * zoom,
            ...
            <span style={{ pointerEvents: "none", display: "block" }}>{ann.text}</span>
          )}
        </div>
      );
    }
              ...
              <div className="flex min-h-full items-start justify-center p-8">
                <div className="relative inline-block shadow-2xl" style={{ width: cW, height: cH }}>
                  ...
                  <div ref={overlayRef}
                    ...
                    style={{ width: cW, height: cH, cursor: overlayCursor }}
                  ...
                  >
                    ...
                    {/* Edit Text mode: blue highlights over detected PDF text */}
                    {tool === "editText" && pdfTextBlocks.map((block, i) => (
                      <div key={`tb-${i}`}
                        className="absolute"
                        style={{
                          left: block.x * zoom, top: block.y * zoom,
                          width: block.w * zoom, height: block.h * zoom,
                        ...
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(66,133,244,0.2)";
                        }}
                        onMouseLeave={(e) => {
                          (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(66,133,244,0.08)";
                        }}
                        onMouseDown={(e) => {
                          e.stopPropagation();
                          editPdfText(block);
