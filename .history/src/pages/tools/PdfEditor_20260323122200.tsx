...
async function extractTextBlocks(bytes: Uint8Array, pageIndex: number, rotation: number): Promise<PdfTextBlock[]> {
  ...
  for (const item of content.items as any[]) {
    ...
    const sx = a * tx[4] + c * tx[5] + e;
    const sy = b * tx[4] + d * tx[5] + f;
    const w = item.width || fontSize * item.str.length * 0.5;
    const h = fontSize * 1.3;
    // Extract color from the PDF text item
    let color = "#000000";
    if (item.color) {
...
export function PdfEditor() {
  ...
  const editPdfText = useCallback((block: PdfTextBlock) => {
    ...
    const ta: TextAnnotation = {
      ...
      color: block.color || "#000000", bold: false, italic: false, underline: false, strikethrough: false, align: "left",
    };
    setAnns((a) => [...a, wo, ta]); setSelId(ta.id); setEditId(ta.id); clearDl();
  }, [pg, clearDl, pushHistory]);
        ...
        >
          {isEd ? (
            <textarea
              ...
              onBlur={() => {
                updateAnn(ann.id, { text: editText });
                pushHistory();
                setEditId(null);
              }}
              onKeyDown={(e) => {
              ...
              style={{
                ...textStyles,
                minHeight: ann.fontSize * zoom * 1.4,
                minWidth: 120 * zoom,
            ...
            <span style={{ pointerEvents: "none" }}>
              {ann.text || <span style={{ color: "#aaa", fontStyle: "italic", fontSize: ann.fontSize * zoom * 0.85 }}>Double-click to edit</span>}
            </span>
          )}
        </div>
      );
        ...
        <div className="flex h-screen flex-col bg-[#f5f5f5] overflow-hidden">
          ...
          {tool === "editText" && !sel && (
            <div className="flex items-center gap-3 border-b border-[#e0e0e0] bg-[#e8f0fe] px-4 py-2 select-none">
              ...
              <span className="text-xs font-medium text-[#4285F4]">Click on any text in the PDF to edit it. Text regions are highlighted in blue.</span>
            </div>
          )}
                ...
                <div className="relative inline-block shadow-2xl" style={{ width: cW, height: cH }}>
                  ...
                  >
                    ...
                    {tool === "editText" && pdfTextBlocks.map((block, i) => (
                      <div key={`tb-${i}`}
                        className="absolute transition-colors hover:bg-blue-500/20"
                        style={{
