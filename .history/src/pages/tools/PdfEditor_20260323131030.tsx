...
export function PdfEditor() {
  ...
  const editPdfText = useCallback((block: PdfTextBlock) => {
    ...
    setSelId(ta.id);
    setEditId(ta.id);
    // Remove this block from highlights so it can't be clicked again
    setPdfTextBlocks((prev) => prev.filter((b) => b !== block));
    // Switch to select so user can edit the new annotation
    setTool("select");
    clearDl();
  }, [pg, clearDl, pushHistory]);
        ...
        >
          {isEd ? (
            <textarea
              ...
              style={{
                ...textStyles,
                minHeight: Math.max(ann.fontSize * zoom * 1.3, 24),
                minWidth: Math.max(100 * zoom, 80),
