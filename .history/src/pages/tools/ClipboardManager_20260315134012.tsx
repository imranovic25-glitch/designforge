...
import { Pin, PinOff, Copy, Trash2, ChevronUp, ChevronDown, Check, Search, RefreshCw, MonitorPlay, MonitorOff, X, GripVertical } from "lucide-react";
...
export function ClipboardManager() {
  ...
  // Move pinned item up/down in priority (keyboard fallback)
  ...
  // Called by Reorder.Group when user drag-drops a new order
  const handlePinnedReorder = (newOrder: ClipItem[]) => {
    setClips(prev => {
      const unpinned = prev.filter(c => !c.pinned);
      const reordered = newOrder.map((item, i) => ({ ...item, pinOrder: i }));
  ...
  /* ── Pinned item with drag handle (uses Reorder.Item) ── */
  function PinnedItem({ item, idx, total }: { item: ClipItem; idx: number; total: number }) {
    ...
    return (
      <Reorder.Item
        ...
        className={`group relative flex gap-2 p-4 rounded-2xl border transition-colors duration-200
          bg-amber-500/[0.04] border-amber-500/15 hover:bg-amber-500/[0.07] cursor-default`}
        style={{ listStyle: "none" }}
        whileDrag={{ scale: 1.02, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 50 }}
      >
        {/* Drag handle */}
        <div
          onPointerDown={e => { e.preventDefault(); dragControls.start(e); }}
          title="Drag to reorder"
          className="flex items-center justify-center shrink-0 w-5 cursor-grab active:cursor-grabbing text-white/20 hover:text-amber-400/60 transition-colors touch-none"
        >
          <GripVertical className="h-4 w-4" />
        </div>

        {/* ↑↓ fallback for keyboard / non-drag users */}
        <div className="flex flex-col gap-0.5 justify-center shrink-0">
          <button
            onClick={() => movePinOrder(item.id, "up")}
            disabled={idx === 0}
            title="Move up"
            className="w-5 h-5 flex items-center justify-center rounded hover:bg-white/10 disabled:opacity-20 transition-colors text-white/30 hover:text-white"
          >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[10px] text-amber-500/60 font-medium">#{idx + 1}</span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-150">
          <button onClick={() => copyItem(item.id, item.text)} title="Copy"
          ...
          <div className="relative" data-transform-menu>
            <button onClick={() => setTransformId(transformId === item.id ? null : item.id)} title="Transform & copy"
              ...
              {transformId === item.id && (
                <motion.div initial={{ opacity: 0, y: -4, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  ...
                  {TRANSFORMS.map(([t, label]) => (
                    <button key={t} onClick={() => applyAndCopy(item.text, t)}
                      className="w-full text-left px-3 py-2 text-sm text-white/55 hover:text-white hover:bg-white/5 transition-colors">{label}</button>
                  ))}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          <button onClick={() => togglePin(item.id)} title="Unpin"
            className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors text-amber-400 hover:text-amber-300">
  ...
  const renderHistoryItem = (item: ClipItem, idx: number) => (
    <motion.div key={item.id} layout initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.2 }}
      className="group relative flex gap-3 p-4 rounded-2xl border transition-colors duration-200 bg-transparent border-white/[0.06] hover:bg-white/[0.025]"
    >
      ...
      <div className="flex items-start gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-150">
        ...
        <button onClick={() => togglePin(item.id)} title="Pin to top"
          className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors text-white/40 hover:text-amber-400">
  ...
  return (
    <ToolLayout
      ...
      icon={<ClipboardIcon />}
      toolSlug="clipboard-manager"
      faqItems={faqItems}
      relatedGuides={[
        { title: "How to Write Markdown", path: "/guides/how-to-write-markdown" },
      ]}
      relatedComparisons={[
        { title: "Best Markdown Editors", path: "/comparisons/best-markdown-editors" },
      ]}
    >
      <SEOHead
        ...
        {pinnedClips.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-white/20 ml-1">Drag ⠿ to reorder · ↑↓ also works</span>
            </div>
            <Reorder.Group
              axis="y"
              values={pinnedClips}
            ...
            >
              {pinnedClips.map((item, idx) => (
                <PinnedItem key={item.id} item={item} idx={idx} total={pinnedClips.length} />
              ))}
            </Reorder.Group>
          </motion.div>
        )}
        ...
        <div>
          ...
          <div className="space-y-2">
            <AnimatePresence mode="popLayout">
              {historyClips.map((item, idx) => renderHistoryItem(item, idx))}
            </AnimatePresence>
          </div>
        </div>
      )}
