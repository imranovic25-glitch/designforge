...
/* Shared canvas for measuring text width */
const _mCtx = (() => { const c = document.createElement("canvas"); return c.getContext("2d"); })();
function measureTW(text: string, font: string): number {
  if (!_mCtx || !text) return 0;
  _mCtx.font = font;
  return Math.max(...text.split("\n").map(l => _mCtx.measureText(l || " ").width));
}
...
export function PdfEditor() {
  ...
  const addText = useCallback((x: number, y: number) => {
    ...
    // Inherit font properties from the last selected text annotation (if any)
    const lastText = sel?.kind === "text" ? sel : null;
    const ann: TextAnnotation = {
      kind: "text", id, pageId: pg.id,
      x, y, text: "",
      fontSize: lastText?.fontSize ?? 14,
      fontFamily: lastText?.fontFamily ?? "Helvetica",
      color: lastText?.color ?? "#000000",
    ...
    textUndoIdx.current = 0;
    setTool("select");
    clearDl();
  }, [pg, sel, clearDl, pushHistory]);
  ...
  const handleTextLayerClick = useCallback((e: React.MouseEvent) => {
    ...
    pushHistory();
    // Whiteout to cover original text with slight padding for complete coverage
    const pad = 3;
  ...
  const onOverlayMove = useCallback((e: React.MouseEvent) => {
    ...
    if (drag) {
      setAnns((a) => a.map((x) => {
        ...
        // Clear PDF-native coordinates when moved so export uses updated screen coords
        if (x.kind === "text") return { ...x, x: nx, y: ny, pdfX: undefined, pdfY: undefined } as Annotation;
        if (x.kind === "whiteout") return { ...x, x: nx, y: ny, pdfX: undefined, pdfY: undefined, pdfW: undefined, pdfH: undefined } as Annotation;
        return { ...x, x: nx, y: ny } as Annotation;
      }));
      return;
    }
    if (resize) {
      setAnns((a) => a.map((ann) => {
        ...
        if (ann.kind === "whiteout") return { ...ann, x: nx, y: ny, w: nw, h: nh, pdfX: undefined, pdfY: undefined, pdfW: undefined, pdfH: undefined } as Annotation;
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (editId) return;
      if ((e.key === "Delete" || e.key === "Backspace") && selId && tool !== "editText") delAnn(selId);
  ...
  }, [editId, selId, tool, delAnn, undo, redo]);
  ...
  const doExport = async () => {
    ...
    try {
      ...
      /* Smart font from PDF font name — detects family+weight+style from original */
      async function getFontFromPdfName(pdfFontName: string | undefined, pdfFontFamily: string | undefined, fallbackFam: FontFamily, b: boolean, it: boolean) {
        let fam: FontFamily = fallbackFam;
        let bold = b;
        let italic = it;
        const src = ((pdfFontName || "") + " " + (pdfFontFamily || "")).toLowerCase();
        // Detect family
        if (src.includes("courier") || src.includes("mono")) fam = "Courier";
        else if (src.includes("times") || (src.includes("serif") && !src.includes("sans"))) fam = "TimesRoman";
        else fam = "Helvetica";
        // Detect weight/style from font name
        if (src.includes("bold")) bold = true;
        if (src.includes("italic") || src.includes("oblique")) italic = true;
        return getFont(fam, bold, italic);
      }
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        for (const ann of pa) {
          if (ann.kind === "whiteout") {
            ...
            // Use smart font detection from original PDF font name when available
            const font = ann.pdfFontName
              ? await getFontFromPdfName(ann.pdfFontName, ann.pdfFontFamily, ann.fontFamily, ann.bold, ann.italic)
              : await getFont(ann.fontFamily, ann.bold, ann.italic);
            ...
            if (ann.pdfX != null && ann.pdfY != null) {
              ...
              // Offset accounts for CSS lineHeight half-leading + font ascent to match preview
              op.drawText(ann.text, { x: ann.x, y: pH - ann.y - ann.fontSize * 0.82, size: ann.fontSize, font, color: rgb(r, g, b) });
            }
  ...
  const renderAnn = (ann: Annotation) => {
    ...
    if (ann.kind === "text") {
      ...
      const textStyles: React.CSSProperties = {
        ...
        // Use original PDF font family CSS if available, fallback to FONT_MAP
        fontFamily: ann.pdfFontFamily ? `${ann.pdfFontFamily}, ${FONT_MAP[ann.fontFamily].css}` : FONT_MAP[ann.fontFamily].css,
        color: ann.color,
      ...
      // Width: measure actual text width for content, small cursor for empty
      const cssFont = `${ann.italic ? "italic " : ""}${ann.bold ? "bold " : ""}${scaledFontSize}px ${textStyles.fontFamily}`;
      const measuredW = hasContent ? measureTW(ann.text, cssFont) + scaledFontSize * 1.5 : 0;
      const baseW = ann.pdfW ? ann.pdfW * zoom + scaledFontSize * 0.5 : measuredW;
      const minW = hasContent ? Math.max(baseW, scaledFontSize * 2) : Math.max(scaledFontSize * 2, 30);
