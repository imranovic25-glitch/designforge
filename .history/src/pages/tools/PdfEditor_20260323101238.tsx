...
type ToolMode = "hand" | "select" | "editText" | "addText" | "draw" | "shape" | "line" | "image" | "highlight" | "redact";
...
type Annotation = TextAnnotation | ImageAnnotation | ShapeAnnotation | HighlightAnnotation | DrawAnnotation;
...
async function buildThumbs(bytes: Uint8Array): Promise<EditorPage[]> {
  ...
  for (let i = 0; i < pdf.numPages; i++) {
    ...
    out.push({
      ...
      width: bv.width, height: bv.height,
      rotation: 0,
    });
  }
  return out;
...
export function PdfEditor() {
  ...
  const load = async (f: File) => {
    ...
    try {
      ...
      setFile(f); setSrcBytes(bytes); setPages(pgs); setPgIdx(0);
  ...
  const movePg = (from: number, dir: -1 | 1) => {
    const to = from + dir;
    if (to < 0 || to >= pages.length) return;
    setPages((p) => { const n = [...p]; [n[from], n[to]] = [n[to], n[from]]; return n; });
    if (pgIdx === from) setPgIdx(to);
    else if (pgIdx === to) setPgIdx(from);
    clearDl();
  };
  ...
  const addText = useCallback((x: number, y: number) => {
    ...
    const a: TextAnnotation = {
      ...
      x, y, text: "Type here", fontSize: 16, fontFamily: "Courier",
      color: "#000000", bold: false, italic: false, underline: false, strikethrough: false, align: "left",
    };
    setAnns((p) => [...p, a]); setSelId(a.id); setEditId(a.id); setTool("select"); clearDl();
  ...
  const onOverlayMove = useCallback((e: React.MouseEvent) => {
    ...
    if (drawStart) { setDrawCur(p); return; }
  ...
  const onOverlayUp = useCallback(() => {
    ...
    if (tool === "draw" && drawPoints.length > 2 && pg) {
      ...
      const a: DrawAnnotation = {
        ...
        x: minX, y: minY, points: drawPoints, color: "#FF0000", width: 2,
      };
    ...
    setDrawPoints([]);
  }, [drag, resize, drawStart, drawCur, drawPoints, tool, shapeKind, pg, clearDl, pushHistory]);
  ...
  const doExport = async () => {
    ...
    try {
      ...
      async function getFont(fam: FontFamily, b: boolean, i: boolean) {
        const k = `${fam}_${b}_${i}`;
        if (fontCache[k]) return fontCache[k];
        let n = StandardFonts.Helvetica;
        if (fam === "Helvetica") n = b && i ? StandardFonts.HelveticaBoldOblique : b ? StandardFonts.HelveticaBold : i ? StandardFonts.HelveticaOblique : StandardFonts.Helvetica;
        else if (fam === "TimesRoman") n = b && i ? StandardFonts.TimesRomanBoldItalic : b ? StandardFonts.TimesRomanBold : i ? StandardFonts.TimesRomanItalic : StandardFonts.TimesRoman;
        else if (fam === "Courier") n = b && i ? StandardFonts.CourierBoldOblique : b ? StandardFonts.CourierBold : i ? StandardFonts.CourierOblique : StandardFonts.Courier;
        ...
        for (const ann of pa) {
          if (ann.kind === "text" && ann.text.trim()) {
            ...
            for (let pi = 1; pi < ann.points.length; pi++) {
              const a = ann.points[pi - 1], b = ann.points[pi];
              op.drawLine({ start: { x: a.x, y: pH - a.y }, end: { x: b.x, y: pH - b.y }, thickness: ann.width, color: rgb(dr, dg, db) });
            }
          }
        }
      }
  ...
  const Handles = ({ id }: { id: string }) => (
    <>
      {(["nw", "ne", "sw", "se"] as const).map((c) => (
        <div key={c} className="absolute z-30 h-[10px] w-[10px] rounded-full border-2 border-[#4285F4] bg-white shadow"
          style={{
  ...
  const renderAnn = (ann: Annotation) => {
    ...
    if (ann.kind === "text") {
      ...
      return (
        <div key={ann.id}
          style={{
            ...base, fontSize: ann.fontSize * zoom, fontFamily: FONT_MAP[ann.fontFamily].css,
            color: ann.color, fontWeight: ann.bold ? 700 : 400, fontStyle: ann.italic ? "italic" : "normal",
            textDecoration: [ann.underline ? "underline" : "", ann.strikethrough ? "line-through" : ""].filter(Boolean).join(" ") || "none",
            textAlign: ann.align, minWidth: 60 * zoom, lineHeight: 1.4, whiteSpace: "pre-wrap", wordBreak: "break-word",
            padding: `${2 * zoom}px ${4 * zoom}px`,
            outline: isSel ? "2px solid #4285F4" : "none", outlineOffset: 2,
            background: isEd ? "rgba(255,255,255,0.95)" : "transparent",
            cursor: tool === "select" || tool === "editText" ? (isEd ? "text" : "move") : undefined,
            zIndex: isSel ? 20 : 10, borderRadius: 2,
          }}
          onMouseDown={(e) => { if (isEd) { e.stopPropagation(); return; } onAnnDown(e, ann.id); }}
          onDoubleClick={(e) => { e.stopPropagation(); setSelId(ann.id); setEditId(ann.id); }}
        >
          {isEd ? (
            <textarea autoFocus defaultValue={ann.text}
              onBlur={(e) => { updateAnn(ann.id, { text: e.target.value }); setEditId(null); pushHistory(); }}
              onKeyDown={(e) => { if (e.key === "Escape") setEditId(null); }}
              className="w-full resize-none border-none bg-transparent p-0 outline-none"
              style={{
                fontSize: ann.fontSize * zoom, fontFamily: FONT_MAP[ann.fontFamily].css,
                color: ann.color, fontWeight: ann.bold ? 700 : 400, fontStyle: ann.italic ? "italic" : "normal",
                textDecoration: [ann.underline ? "underline" : "", ann.strikethrough ? "line-through" : ""].filter(Boolean).join(" ") || "none",
                textAlign: ann.align, lineHeight: 1.4, minHeight: ann.fontSize * zoom * 2, minWidth: 140 * zoom,
              }}
            />
          ) : <span>{ann.text}</span>}
        </div>
    ...
    if (ann.kind === "image") {
      return (
        <div key={ann.id} style={{ ...base, width: ann.w * zoom, height: ann.h * zoom, outline: isSel ? "2px solid #4285F4" : "none", outlineOffset: 2, cursor: "move", zIndex: isSel ? 20 : 10 }}
    ...
    if (ann.kind === "shape") {
      return (
        <div key={ann.id} style={{
          ...
          outline: isSel ? "2px solid #4285F4" : "none", outlineOffset: 2,
          boxSizing: "border-box", cursor: "move", zIndex: isSel ? 20 : 10,
        }} onMouseDown={(e) => onAnnDown(e, ann.id)}>
          {isSel && <Handles id={ann.id} />}
        </div>
      );
    ...
    if (ann.kind === "draw" && ann.points.length > 1) {
      ...
      return (
        <svg key={ann.id} style={{ position: "absolute", left: (minX - ann.width) * zoom, top: (minY - ann.width) * zoom, width: sw * zoom, height: sh * zoom, cursor: "move", zIndex: isSel ? 20 : 10, outline: isSel ? "2px solid #4285F4" : "none", outlineOffset: 2, pointerEvents: "all" }}
          onMouseDown={(e) => onAnnDown(e, ann.id)}>
  ...
  return (
    <>
      ...
      {!file ? (
        ...
        /* ══════════════ EDITOR WORKSPACE ══════════════ */
        <div className="flex h-screen flex-col bg-[#f5f5f5] overflow-hidden select-none">

          {/* ───── TOP HEADER BAR ───── */}
          <div className="flex items-center justify-between border-b border-[#e0e0e0] bg-white px-4 py-2 shadow-sm">
          ...
          <div className="flex items-center gap-1 border-b border-[#e0e0e0] bg-white px-3 py-1.5">
            ...
            <Divider />
            <TBtn active={tool === "editText"} onClick={() => setTool("editText")} icon={<Type className="h-[18px] w-[18px]" />} label="Edit Text" className={tool === "editText" ? "!bg-[#4285F4] !text-white" : ""} />
            ...
            <TBtn active={tool === "redact"} onClick={() => setTool("redact")} icon={<Eraser className="h-4 w-4" />} label="Redact" />
            <Divider />
          ...
          {/* ───── SECONDARY TOOLBAR (context) ───── */}
          {(tool === "editText" || tool === "addText") && sel?.kind === "text" && (() => {
            const t = sel as TextAnnotation;
            return (
              <div className="flex items-center gap-2 border-b border-[#e0e0e0] bg-[#fafbfc] px-4 py-1.5">
                ...
                <select value={t.fontSize} onChange={(e) => updateAnn(t.id, { fontSize: Number(e.target.value) })}
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="hidden w-[160px] shrink-0 flex-col overflow-y-auto border-r border-[#e0e0e0] bg-[#f8f8fa] py-3 md:flex">
              {pages.map((p, i) => (
                <div key={p.id} className="px-3 mb-1">
                  ...
                  <div className="mt-1 flex items-center justify-center gap-1">
                    <button onClick={() => dupPg(i)} className="rounded p-1 text-[#999] transition-colors hover:bg-[#eee] hover:text-[#555]" title="Duplicate">
                      <Copy className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => rotatePg(i, 90)} className="rounded p-1 text-[#999] transition-colors hover:bg-[#eee] hover:text-[#555]" title="Rotate">
                      <RotateCw className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => delPg(i)} disabled={pages.length <= 1} className="rounded p-1 text-[#999] transition-colors hover:bg-red-50 hover:text-red-500 disabled:opacity-30" title="Delete">
              ...
              <div className="px-3 mt-2">
                <div className="flex h-16 items-center justify-center rounded-lg border-2 border-dashed border-[#ddd] text-[#bbb]">
            ...
            {/* ── CENTER: CANVAS ── */}
            <div className="relative flex-1 overflow-auto bg-[#e8e8ec]"
              ...
              <div className="flex min-h-full items-start justify-center p-8">
                <div className="relative inline-block shadow-2xl" style={{ width: cW, height: cH }}>
                  <canvas ref={canvasRef} className="block rounded-sm bg-white" style={{ width: cW, height: cH }} />

                  {/* annotation overlay */}
                  <div ref={overlayRef}
                    className={`absolute inset-0 ${
                      tool === "addText" ? "cursor-text" :
                      tool === "shape" || tool === "highlight" || tool === "redact" ? "cursor-crosshair" :
                    ...
                    {tool === "draw" && drawPoints.length > 1 && (
                      <svg className="pointer-events-none absolute inset-0" width={cW} height={cH}>
                        <path
                          ...
                          fill="none" stroke="#FF0000" strokeWidth={2 * zoom} strokeLinecap="round" strokeLinejoin="round"
                        />
                      </svg>
                    )}
            ...
            <div className="hidden w-[260px] shrink-0 flex-col overflow-y-auto border-l border-[#e0e0e0] bg-white lg:flex">
                ...
                {sel?.kind === "text" && (() => {
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-[#999]">Current Color</label>
                  ...
                  return (
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-[#666]">{Math.round(im.w)} × {Math.round(im.h)} pt</p>
                  ...
                  <>
                    <div className="rounded-xl bg-[#f8f8fa] p-4">
                      ...
                      <ul className="space-y-1.5 text-[11px] text-[#888] leading-relaxed">
                        <li><strong>Edit Text:</strong> Click existing text elements</li>
                        ...
                        <li><strong>Redact:</strong> Drag to black-out areas</li>
                        <li><strong>Ctrl+Z / Ctrl+Y:</strong> Undo / Redo</li>
                        <li><strong>Delete / Backspace:</strong> Remove selected</li>
                      </ul>
                    </div>
