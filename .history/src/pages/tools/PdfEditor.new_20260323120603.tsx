...
async function extractTextBlocks(bytes: Uint8Array, pageIndex: number, rotation: number): Promise<PdfTextBlock[]> {
  ...
  for (const item of content.items as any[]) {
    ...
    blocks.push({ str: item.str, x: sx, y: sy - h, w, h, fontSize });
  }
  return blocks;
}
...
export function PdfEditor() {
  ...
  const addText = useCallback((x: number, y: number) => {
    ...
    const a: TextAnnotation = {
      ...
      x, y, text: "", fontSize: 16, fontFamily: "Helvetica",
      ...
      return (
        <div key={ann.id}
          style={{
            ...
            outline: isSel ? "2px solid #4285F4" : isEd ? "2px solid #34A853" : "none",
            outlineOffset: 2,
          ...
          {isEd ? (
            <textarea
              ...
              onKeyDown={(e) => {
                e.stopPropagation();
                if (e.key === "Escape") {
                  updateAnn(ann.id, { text: (e.target as HTMLTextAreaElement).value });
                  setEditId(null);
                }
              }}
            ...
            <span style={{ pointerEvents: "none" }}>
              {ann.text || <span className="text-gray-400 italic">Double-click to edit</span>}
            </span>
          )}
        </div>
      );
