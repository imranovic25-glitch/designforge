...
export function PdfEditor() {
  ...
  const renderAnn = (ann: Annotation) => {
    ...
    if (ann.kind === "text") {
      ...
      // Minimal container size: just enough for the font, NOT a big fixed min-width
      const minH = Math.max(scaledFontSize * 1.2, 10);
      // For new empty text (addText), give a small min-width so it's clickable
      const minW = hasContent ? 0 : Math.max(scaledFontSize * 3, 20);
          ...
          {isEd ? (
            <textarea
              ...
              onChange={(e) => {
                ...
                // Auto-resize fallback: shrink then grow to content
                const ta = e.target;
                ta.style.height = "0";
                ta.style.height = ta.scrollHeight + "px";
                ta.style.width = "0";
                ta.style.width = Math.max(ta.scrollWidth, hasContent ? 0 : scaledFontSize * 6) + "px";
              }}
              onBlur={() => {
                pushHistory();
              ...
              className="block resize-none border-none bg-transparent outline-none"
              style={{
                ...textStyles,
                minHeight: minH,
                width: hasContent ? "auto" : Math.max(scaledFontSize * 6, 40),
                ...
                overflow: "hidden",
                resize: "none",
                // Auto-size: field-sizing makes textarea grow with content
                fieldSizing: "content",
              } as React.CSSProperties}
            />
          ) : (
            <span style={{ pointerEvents: "none", display: "block", whiteSpace: "pre" }}>{ann.text}</span>
          )}
