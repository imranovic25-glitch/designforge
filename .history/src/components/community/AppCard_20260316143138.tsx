  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {app.tier !== "free" && (
              <span className="text-[10px] uppercase tracking-wider font-bold text-white/25 bg-white/[0.04] border border-white/[0.06] px-1.5 py-0.5 rounded-md">
      ...
      {(() => {
        ...
        const pct = app.tester_slots > 0 ? Math.min(100, (app.slots_filled / app.tester_slots) * 100) : 0;
        return (
          <div className="px-4 pb-3 pt-1">
            <div className="flex items-center justify-between mb-1.5">
              <span className="flex items-center gap-1.5 text-[11px] text-white/30">
                <Users size={11} />
                {app.slots_filled}/{app.tester_slots} testers
              </span>
              {isFull && (
                <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400">FULL</span>
              )}
            </div>
            <div className="h-1 bg-white/[0.05] rounded-full overflow-hidden">
              <div
                style={{ width: `${pct}%` }}
                className={`h-full rounded-full transition-all duration-500 ${isFull ? "bg-amber-400/50" : "bg-emerald-500/50"}`}
              />
            </div>
          </div>
        );
