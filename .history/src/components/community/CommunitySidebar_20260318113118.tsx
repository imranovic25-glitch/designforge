...
import {
  ...
  Heart, ThumbsUp, Share2, ArrowUp, Flag, Gift, Check, MessageCircle, MessageSquare,
} from "lucide-react";
...
export function CommunitySidebar({ sort = "newest", onSortChange, onResetFilters, repoBalance }: CommunitySidebarProps) {
  ...
  return (
    <nav className="py-3 px-2 space-y-0.5">
      ...
      <button onClick={() => handleSort("most-liked")} className={navCls(isOnHub && sort === "most-liked")}>
        <ThumbsUp size={18} className={(isOnHub && sort === "most-liked") ? "" : "text-blue-400/60"} /> Top Liked
      </button>
      <button onClick={() => handleSort("most-loved")} className={navCls(isOnHub && sort === "most-loved")}>
        <Heart size={18} className={(isOnHub && sort === "most-loved") ? "" : "text-pink-400/60"} /> Top Loved
      </button>
      <button onClick={() => handleSort("most-feedback")} className={navCls(false)}>
        <MessageSquare size={18} className="text-emerald-400/60" /> Top Commented
      </button>
