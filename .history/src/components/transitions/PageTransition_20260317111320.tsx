  ...
  const tiles = useMemo<TileData[]>(() => {
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        ...
        if (light) {
          /* Neutral dark-gray for light theme — inverts cleanly */
          const v = Math.round(6 + intensity * 18);
          r = g = b = v;
        } else {
          /* Deep violet gradient for dark theme */
          r = Math.round(8 + intensity * 30);
          g = Math.round(4 + intensity * 8);
          b = Math.round(14 + intensity * 56);
        }
        arr.push({
          row,
          col,
    ...
    <div className="fixed inset-0 z-[100] pointer-events-none overflow-hidden">
      ...
      <motion.div
        ...
        style={{
          ...
          backgroundColor: light ? "rgba(0, 0, 0, 0.55)" : "rgba(5, 2, 16, 0.55)",
        }}
        initial={{ opacity: 1 }}
        animate={{ opacity: 0 }}
      ...
      <motion.div
        ...
        style={{
          background: light
            ? "linear-gradient(135deg, transparent 0%, transparent 44%, rgba(255,255,255,0.03) 47%, rgba(255,255,255,0.12) 50%, rgba(255,255,255,0.03) 53%, transparent 56%, transparent 100%)"
      ...
      {/* Secondary shimmer (slightly delayed) */}
      <motion.div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: light
            ? "linear-gradient(135deg, transparent 0%, transparent 46%, rgba(255,255,255,0.02) 49%, rgba(255,255,255,0.06) 50%, rgba(255,255,255,0.02) 51%, transparent 54%, transparent 100%)"
            : "linear-gradient(135deg, transparent 0%, transparent 46%, rgba(120,80,220,0.03) 49%, rgba(180,140,255,0.1) 50%, rgba(120,80,220,0.03) 51%, transparent 54%, transparent 100%)",
          willChange: "transform",
        }}
        initial={{ x: "-100%", y: "-100%" }}
        animate={{ x: "100%", y: "100%" }}
