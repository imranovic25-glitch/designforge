...
const FRAG = `
  ...
  vec2 blobPos(int i, float t) {
    ...
    float rx = 0.28 + fi * 0.035;
    float ry = 0.22 + fi * 0.04;
    return vec2(
      0.5 + rx * sin(t * speed + fi * 1.7),
      0.5 + ry * cos(t * speed * 0.75 + fi * 2.3)
    );
  }

  float blobRadius(int i, float t) {
    float fi = float(i);
    float base = 0.10 + fi * 0.012;
    return base + 0.015 * sin(t * 0.4 + fi * 3.0);
  }
  ...
  void main() {
    ...
    // Colour: white-to-translucent with subtle blue shift in the thickest blobs
    float core = smoothstep(3.5, 6.0, field);
    vec3 col = mix(
      vec3(1.0, 1.0, 1.0),                 // outer glow — pure white
      vec3(0.85, 0.90, 1.0),               // inner core — faint cool blue
      core
    );

    // Luminance modulation — glow fades toward edge
    float alpha = edge * (0.06 + 0.06 * core);
...
const DROPS: {
  ...
  { size: 340, x: "15%", y: "20%", delay: 0, duration: 18, opacity: 0.07 },
  { size: 260, x: "70%", y: "35%", delay: 2, duration: 22, opacity: 0.05 },
  { size: 200, x: "40%", y: "65%", delay: 4, duration: 16, opacity: 0.06 },
  { size: 180, x: "85%", y: "15%", delay: 1, duration: 20, opacity: 0.04 },
  { size: 300, x: "55%", y: "50%", delay: 3, duration: 24, opacity: 0.06 },
  { size: 140, x: "25%", y: "75%", delay: 5, duration: 14, opacity: 0.05 },
];
...
const RIPPLES = [
  { delay: 0, scale: 1, opacity: 0.15 },
  { delay: 1.2, scale: 1.6, opacity: 0.1 },
  { delay: 2.8, scale: 2.2, opacity: 0.06 },
];
