...
export function ArticleLayout({
  ...
  // Auto-generate breadcrumbs from category/title if not provided
  const breadcrumbs = breadcrumbsProp ?? [
    { label: category, href: categoryLink ?? `/${category.toLowerCase()}` },
    { label: title },
  ];
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <RevealOnScroll className="mb-12">
          {breadcrumbs && breadcrumbs.length > 0 ? (
            <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm font-medium text-white/40">
            ...
            <Link
              to={categoryLink ?? `/${category.toLowerCase()}`}
