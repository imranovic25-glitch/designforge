...
const DEFAULT_OG_IMAGE = absolutePublicAssetUrl("/og-default.png");
...
function buildWebSiteSchema() {
  return {
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: absolutePublicAssetUrl("/favicon.svg"),
      },
    },
  };
}
...
function buildCollectionPageSchema(title: string, description: string, canonicalUrl: string) {
  return {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: title,
...
function buildReviewSchemas(
  ...
  return items.map((r) => ({
    ...
    itemReviewed: {
      "@type": "SoftwareApplication",
      name: targetName,
      url: targetUrl.startsWith("http") ? targetUrl : `${SITE_URL}${targetUrl}`,
    },
  }));
}
...
export function SEOHead(props: SEOHeadProps) {
  ...
  if (schema === "CollectionPage") {
    schemas.push(buildCollectionPageSchema(title, description, canonicalUrl));
  }
  if (schema === "Article") {
  ...
  // HowTo steps — emit independently so it works alongside any main schema
  if (props.howToSteps?.length && schema !== "HowTo") {
    schemas.push(buildHowToSchema(props, canonicalUrl));
  }
  ...
  // Individual Review items (typically paired with aggregateRating)
  if (props.reviewItems?.length && props.reviewTargetName && props.reviewTargetUrl) {
    schemas.push(...buildReviewSchemas(props.reviewItems, props.reviewTargetName, props.reviewTargetUrl));
  }
