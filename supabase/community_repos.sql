...
BEGIN
  ...
  -- Require meaningful feedback to reduce low-effort farming.
  IF p_feedback_text IS NULL OR char_length(trim(p_feedback_text)) < 40 THEN
  ...
  -- Determine whether this is the user's first feedback on this submission.
  SELECT id
  ...
  v_inserted := v_existing_feedback_id IS NULL;
  ...
  IF v_inserted THEN
    ...
    BEGIN
      ...
      VALUES (v_uid, 1, 'feedback_award', p_submission_id, v_feedback.id);
...
-- ═══════════════════════════════════════════
-- 8. Generic community action repo rewards
--    Prevents duplicate rewards per user/submission/action
--    Actions: upvote(1), like(1), love(1), share(2), report(1)
-- ═══════════════════════════════════════════

CREATE TABLE IF NOT EXISTS community_action_rewards (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  submission_id UUID NOT NULL REFERENCES app_submissions(id) ON DELETE CASCADE,
  action_type TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE(user_id, submission_id, action_type)
);

ALTER TABLE community_action_rewards ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can read own action rewards" ON community_action_rewards;
CREATE POLICY "Users can read own action rewards"
  ON community_action_rewards FOR SELECT
  USING (auth.uid() = user_id);
...
BEGIN
  ...
  -- Determine reward amount per action
  CASE p_action
  ...
  -- One reward per action per submission per user
  BEGIN
