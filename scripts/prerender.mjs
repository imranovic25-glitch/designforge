...
async function renderRoute(browser, route) {
  ...
  try {
    ...
    /* Clean up duplicate meta/link/title tags: react-helmet-async appends
       page-specific tags AFTER the originals from index.html. We keep only the
       LAST occurrence of each (i.e. the helmet-injected, page-accurate one). */
    await page.evaluate(() => {
      function dedup(selector, keyFn) {
        const map = new Map();
        ...
        for (const els of map.values()) {
          if (els.length > 1) els.slice(0, -1).forEach((e) => e.remove());
        }
      }

      dedup("meta[name]", (el) => `name:${el.getAttribute("name")}`);
      dedup("meta[property]", (el) => `prop:${el.getAttribute("property")}`);
      dedup('link[rel="canonical"]', () => "canonical");

      /* react-helmet-async updates document.title via the JS API but the
         <title> DOM element may not reflect it.  Sync them explicitly. */
